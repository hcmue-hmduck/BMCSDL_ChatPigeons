...
**Ứng dụng nhắn tin và cuộc gọi thời gian thực, kết nối mọi lúc mọi nơi với kiến trúc hiện đại!**
...
│   │   │   ├── webComponent/   # UI components (Messages, Call, etc.)
...
- **Reaction & Pin**: Tương tác với tin nhắn bằng emoji và ghim các tin nhắn quan trọng.

### Cuộc gọi Video/Audio
- **LiveKit Integration**: Tích hợp SDK LiveKit cho các cuộc gọi thoại và video (1-1 và nhóm) ổn định, chất lượng cao.
- **Tín hiệu thời gian thực**: Xử lý tín hiệu gọi, từ chối, kết thúc cuộc gọi đồng bộ trên nhiều thiết bị.

## Công nghệ sử dụng

### Frontend Stack
- **Angular 21**: Tận dụng triệt để sức mạnh của Signals cho hiệu năng vượt trội.
- **Tailwind CSS 4**: Thiết kế giao diện theo phong cách hiện đại.
- **RxJS**: Quản lý luồng dữ liệu phức tạp và tối ưu hóa các sự kiện socket.
- **Socket.io Client**: Kết nối thời gian thực mượt mà.

### Backend Stack
- **Node.js & Express**: Nền tảng server JavaScript mạnh mẽ và linh hoạt.
- **Sequelize & Mongoose**: Hỗ trợ cả SQL (PostgreSQL/Local) và NoSQL (MongoDB).
...
- **Cơ sở dữ liệu**: PostgreSQL (Sequelize) và MongoDB (Mongoose).
...
cd server
npm install
# Tạo file .env dựa trên file .env.example (Database, Cloudinary, LiveKit,...)
npm run dev
```
...
Ứng dụng sẽ khả dụng tại: `https://localhost:5200`.

---

<div align="center">

Dự án này được thiết kế để cung cấp trải nghiệm nhắn tin an toàn và tốc độ cao ^_^ 
